...
- Codex-specific rendering:
  ...
  - Per-cluster rollup banners (e.g. *"Created 7 files, edited 2 files, ran 2 commands, used the browser and Node Repl"*) emitted before each cluster of tool calls, matching the summary line Codex's UI shows. A "cluster" spans the tool activity between two consecutive assistant messages — but a cluster that contains any patch is further split at each `view_image`, matching how Cursor's UI emits a fresh banner whenever it renders an image mid-edit. The `node_repl` MCP server is labeled "the browser" when `result.Ok._meta["codex/browserUse"]` is set (the same flag Codex itself uses) and "Node Repl" otherwise. Banners emit when the section either touches a patch (even a single file) or has at least two non-patch tools.
  - **WebSearch tool support**. `event_msg/web_search_end` events render inline as `Searched web: "<query>"` for `search` actions or `Opened page: <url>` for `open_page` actions. The paired `response_item/web_search_call` is suppressed to avoid double-printing. Banners include a *"searched web N times"* clause that counts only events with actual content (empty `open_page` placeholders are skipped, matching Cursor's UI).
  - `view_image` sub-cluster split widened to fire when the cluster contains *either* a `patch_apply_end` *or* an `image_generation_end`. Codex's UI splits the rollup whenever the cluster produced tangible output (file changes or a generated image) and the model then alternates with screenshots, but folds pure shell-only turns with an inspection screenshot into a single banner.
