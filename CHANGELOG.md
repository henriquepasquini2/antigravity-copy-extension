...
- Codex-specific rendering:
  ...
  - Per-cluster rollup banners (e.g. *"Created 7 files, edited 2 files, ran 2 commands, used the browser and Node Repl"*) emitted before each cluster of tool calls, matching the summary line Codex's UI shows. A "cluster" spans the tool activity between two consecutive assistant messages. The `node_repl` MCP server is labeled "the browser" when `result.Ok._meta["codex/browserUse"]` is set (the same flag Codex itself uses) and "Node Repl" otherwise. Single-tool clusters get no banner, matching the UI.
